...
func TestCreateTransactionRejectsForeignCategory(t *testing.T) {
	...
	// Limit is left unset, which the query reads as SQL NULL and Postgres as
	// LIMIT ALL: this has to see every row the test could have created, since
	// an empty result would satisfy the "nothing leaked" assertion below no
...
func TestCreateTransactionOOBTotalsUseActiveMonthFromHXCurrentURL(t *testing.T) {
	...
	// The active (past) month now holds both transactions, so its count is 2.
	// A handler still using currentMonthRange() would report today's month
	// instead, which has no transactions for this fresh user (0 count).
