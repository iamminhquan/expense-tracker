...
**Email ingestion** (`internal/inbound`, `internal/handlers/inbox_webhook.go`, `emailworker/`): a Cloudflare Email Worker receives mail forwarded to `<token>@in.<domain>`, signs the JSON body with HMAC-SHA256 under `INBOUND_WEBHOOK_SECRET`, and POSTs it to `/inbox/{token}` — a public route, and the one route exempt from CSRF, because the caller is the Worker rather than a browser with a cookie to double-submit. Three layers decide whether to trust it: the token names an account, the signature proves the body came from our own Worker, and the sender domain proves the mail came from a bank; a message failing only the last is stored as `ignored` rather than dropped, so "why did nothing happen" stays answerable. The handler stores the raw message and answers 200 without reading it; reading happens afterwards in a goroutine, so a parser that is still wrong can never lose an email — the message is already saved and can be replayed. `internal/inbound` holds every rule the Go side shares with the Worker: the payload field names, the HMAC signature, the inbox token, and the body cap. Change one there and `emailworker/src/index.js` changes in the same commit, or email silently stops arriving. `internal/bankmail` is the other half of the reading: it turns `(from, subject, body)` into a `Notice` and touches neither database nor network, so every rule about reading an email is testable without Postgres — the same split `internal/csvimport` draws. `bankmail.NoteKey` is exported because two places must obey one rule (processing an email, and a user correcting a category later); two subtly different rules would leave hints that never match. **What it can read today is narrow on purpose**: MB's transfer-confirmation template, debit side only, because that is the only shape a real notice has arrived in. TPBank and incoming-credit notices are not written rather than guessed — a guessed parser puts invented money in a real ledger, whereas an unrecognised shape becomes a `failed` row that is visible and replayable. `parseMBAmount` validates the shape of an amount instead of coping with variants, for the same reason: this parser faces one sender emitting one format, so tolerance buys nothing and costs a silent wrong amount the day the template changes. `internal/handlers/inbox_process.go` is the only place that writes: a goroutine kicked from the webhook (on `context.Background()`, since the request's context dies with the request) walks every `pending` row for that user, claims each with `UPDATE ... WHERE status='pending' RETURNING` so two goroutines cannot both process one message, and closes the row as `imported`, `ignored` or `failed`. `ignored` means nothing is wrong — an unknown sender or a marketing email; `failed` means our own bug, and is the list worth replaying. Mixing the two would flood the list nobody then reads. Which category the transaction lands in is decided by `category_hints`, a per-account table keyed by `bankmail.NoteKey(description)`: a hit decides it, a miss falls back to the `other`/`other_income` slug. The table is written by an action the owner already performs — correcting the category on a row marked `auto` — so there is no new UI for teaching, and only rows with `source='email'` teach. **A hint decides where a transaction lands, never whether it exists**: a hint lookup that errors falls through to the fallback rather than abandoning the email, because a row in the wrong category costs two clicks to fix while a row that was never created is one the owner cannot know is missing. A hint pointing at a category of the wrong type, or at one belonging to another account, is ignored the same way — `category_hints.category_id` cascades on delete, deliberately unlike `transactions.category_id`, because a hint aimed at a deleted category means nothing while a transaction must never vanish. The token uses lowercase base32 because mail systems normalise the local part of an address to lowercase; base64url would have been case-sensitive and unmatchable after a real round trip.
